...
function promoCardHtml(promo) {
  ...
  const groupControl = action === "product_promotion"
    ? `<div class="group-control"><label><input type="checkbox" class="group-checkbox" onchange="promoToolUpdateCard(promoToolCard(this)); promoToolRefreshStats();" /> סמן כמבצע קבוצה / מיקס עתידי</label><div class="hint">הבחירות יישמרו ב־JSON כקבוצת מוצרים. סקריפט ההעלאה הנוכחי לא יכניס אותן כ־14 מבצעים נפרדים, אלא ישאיר אותן מוכנות למימוש תמיכת קבוצות.</div></div>`
    : "";
...
<head>
  ...
  <script>
    ...
    function promoToolNormalizeProductText(s) { return String(s||'').toLowerCase().replace(/[״”“׳’‘'"־–—\-+₪%.,:;()[\]\/\\]/g,' ').replace(/\s+/g,' ').trim(); }
    function promoToolParseInputValue(value) {
      ...
      if(contains.length>1) { return { error:'multiple', count:contains.length, options:contains.slice(0,8).map(function(p){ return p.id+' | '+p.name; }) }; }
      return null;
    }
    function promoToolAddToCard(card, id, name) { if(!card || !id) return; var product=PROMO_TOOL_PRODUCTS_BY_ID[Number(id)]; var finalName=name || (product ? promoToolProductDisplayName(product) : ('ID '+id)); var list=card.querySelector('.selected-list'); if(!list) return; var existing=list.querySelector('[data-selected-product-id="'+id+'"]'); if(existing) { promoToolToast('המוצר כבר נבחר למבצע הזה'); return; } var empty=list.querySelector('.empty'); if(empty) empty.remove(); var row=document.createElement('div'); row.className='selected-row'; row.setAttribute('data-selected-product-id', String(id)); row.setAttribute('data-selected-product-name', String(finalName)); row.innerHTML='<div><div class="product-name">'+promoToolEscape(finalName)+'</div><div class="product-meta">ID '+promoToolEscape(id)+'</div></div><button type="button" class="small danger" onclick="promoToolRemoveSelected(this)">הסר</button>'; list.appendChild(row); promoToolUpdateCard(card); promoToolRefreshStats(); promoToolToast('נבחר: '+finalName); }
    ...
    function promoToolAddFromInput(btn) { var card=promoToolCard(btn); var input=card && card.querySelector('.product-input'); var parsed=promoToolParseInputValue(input && input.value); if(!parsed) { alert('לא הצלחתי למצוא מוצר מתאים. בחר שורה מההשלמה האוטומטית, או הקלד ID של מוצר.'); return; } if(parsed.error==='multiple') { alert('נמצאו '+parsed.count+' מוצרים מתאימים. כדי לא לבחור מוצר לא נכון, בחר מהרשימה האוטומטית או הקלד ID מדויק.\n\nדוגמאות:\n'+parsed.options.join('\n')); return; } promoToolAddToCard(card, parsed.id, parsed.name); if(input) input.value=''; }
    ...
    function promoToolUpdateCard(card) { if(!card) return; var count=card.querySelectorAll('.selected-row').length; var group=!!(card.querySelector('.group-checkbox') && card.querySelector('.group-checkbox').checked); card.classList.toggle('mapped', count>0); card.classList.toggle('group', group && count>0); var state=card.querySelector('.state'); if(state) state.textContent=count>0 ? ((group?'קבוצה עתידית: ':'מופה: ')+count+' מוצר'+(count>1?'ים':'')) : 'לא מופה'; }
    ...
    function promoToolBuildMapping() { var mappings=[]; document.querySelectorAll('.card').forEach(function(card){ var ids=[]; card.querySelectorAll('.selected-row').forEach(function(row){ ids.push(Number(row.getAttribute('data-selected-product-id'))); }); if(!ids.length) return; var noteEl=card.querySelector('.note'); var group=!!(card.querySelector('.group-checkbox') && card.querySelector('.group-checkbox').checked); var action=card.getAttribute('data-action')||'product_promotion'; mappings.push({ reward_id:Number(card.getAttribute('data-reward-id')), title:card.getAttribute('data-title')||'', type:card.getAttribute('data-type')||'', action: group ? 'promotion_group' : action, mapping_mode: group ? 'group' : 'single_product', is_group_promotion: group, product_ids:ids, note:(noteEl && noteEl.value)||'' }); }); return { source:'${SOURCE}', shop_id:${Number(shopId)}, based_on_report:${JSON.stringify(reportPath)}, generated_at:new Date().toISOString(), mappings:mappings }; }
...
<body>
  ...
  <main>
    <div class="stats">
      ...
      <div class="stat"><strong id="groupCount">0</strong><span>קבוצות עתידיות</span></div>
    </div>

    <div id="toast" class="toast"></div>
    <div class="notice">בכל כרטיס מופיע המבצע מהאקסל, שם המוצר/החיפוש שחולץ ממנו, ושדה כתיבה עם השלמה אוטומטית מתוך מוצרי ה־DB. אפשר לבחור כמה מוצרים לאותו מבצע, ואפשר לסמן שהבחירה היא קבוצת מיקס עתידית שלא תועלה עדיין כמבצעים רגילים.</div>
